...
| `hb.worker_daily_task_actuals` | one worker/task/day actual or summary | worker app, backfill, HB build | Actual logged minutes, WIP timer minutes, daily summaries, and capacity flags. A newer Asana reopen clears only its live completion state; prior minutes and the Asana posting watermark remain intact. |
