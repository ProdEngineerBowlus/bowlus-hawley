...
$port = __PORT__
$out = Join-Path $repo "runtime-output\hawley-worker-page.out.log"
$err = Join-Path $repo "runtime-output\hawley-worker-page.err.log"
...
$env:HAWLEY_WORKER_PORT = [string]$port
...
'@
  ...
  $result = Invoke-SSHCommand -SessionId $session.SessionId -Command "powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand $encoded" -TimeOut 90
  $result.Output
  if ($result.Error) {
    $result.Error
  }
  if ($result.ExitStatus -ne 0) {
    throw "Remote worker page start failed with exit status $($result.ExitStatus)."
  }
} finally {
